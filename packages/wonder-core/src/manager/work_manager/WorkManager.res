module ParsePipelineData = {
  ...
  let _buildJobStream = (execFunc, repo): WonderBsMost.Most.stream<unit> => {
    open WonderBsMost.Most
    execFunc->just->flatMap(func => func(_getStates(), repo), _)->map(_setStates, _)
  }
    ...
    getExecFuncs->WonderCommonlib.ListSt.length === 0
      ? WonderCommonlib.Exception.throwErr(
          WonderCommonlib.Exception.buildErr(
            j`can't get execFunc with pipelineName:${pipelineName}, jobName:${jobName}`,
          ),
        )
      : {
  ...
  let _buildJobStreams = (
    ...
    ->WonderCommonlib.ListSt.reduce(list{}, (streams, {name, type_}: element) =>
      ...
      | #job =>
        ...
        streams->WonderCommonlib.ListSt.push(_buildJobStream(execFunc, repo))
      | #group =>
        let group = _findGroup(name, groups)
        let stream = buildPipelineStreamFunc(getExecFuncs, repo, pipelineName, group, groups)
  ...
  let parse = (state, getExecFuncs, repo, {name, groups, first_group}): WonderBsMost.Most.stream<
    POType.po,
  > => {
    let group = _findGroup(first_group, groups)

    state->POContainer.setState

    _buildPipelineStream(getExecFuncs, repo, name, group, groups)->WonderBsMost.Most.map(
      () => POContainer.unsafeGetState(),
      _,
    )
  }
...
let registerPlugin = (
  {allRegisteredWorkPluginData} as state: POType.po,
...
let init = ({allRegisteredWorkPluginData} as state: POType.po): POType.po => {
...
module MergePipelineData = {
  ...
  let _findAllSpecificPipelineRelatedData = (
    ...
    ->WonderCommonlib.Result.bind(allRegisteredWorkPluginData => {
      allRegisteredWorkPluginData
      ->WonderCommonlib.ListSt.filter((({allPipelineData} as registeredWorkPluginData, _)) => {
        allPipelineData->WonderCommonlib.ArraySt.length === 1
      })
      ->WonderCommonlib.ListSt.map(((
        ...
        (pluginName, getExecFunc, allPipelineData[0], jobOrders->WonderCommonlib.ArraySt.getFirst)
      })
      ->WonderCommonlib.ListSt.traverseResultM(((
        pluginName,
        getExecFunc,
      ...
      : {
          ...
          ->WonderCommonlib.Result.bind(((sameLevelTreeList, _)) => {
            sameLevelTreeList->WonderCommonlib.ListSt.length !== 1
              ...
              : {
                  sameLevelTreeList->WonderCommonlib.ListSt.head->WonderCommonlib.OptionSt.get
                }
          })
        }
  }
...
let runPipeline = (
  {allRegisteredWorkPluginData, states} as state: POType.po,
  pipelineName: PipelineType.pipelineName,
): WonderCommonlib.Result.t2<WonderBsMost.Most.stream<POType.po>> => {
  // TODO check is allRegisteredWorkPluginData duplicate

  WonderCommonlib.Tuple2.collectResult(
    allRegisteredWorkPluginData->MergePipelineData.merge(pipelineName),
    SceneGraphRepoManager.getSceneGraphRepo(state)
    ->WonderCommonlib.OptionSt.map((sceneGraphRepo): PipelineType.repo => {
      {
        sceneGraphRepo: sceneGraphRepo,
      }
    })
    ->WonderCommonlib.OptionSt.get,
  )->WonderCommonlib.Result.mapSuccess((((getExecFuncs, pipelineData), repo)) => {
    ParsePipelineData.parse(state, getExecFuncs, repo, pipelineData)
  })
}
